...
const { initRedisSubscribe } = require("../utils/Redis");


async function createProject(req,res){
    try {
        const { user_id,git_url, project_slug, frame_work, env, node_module_cmd, build_cmd } = req.body;
        let response = await containerBuilderServerJob(git_url, project_slug, process.env.ACCESS_TOKEN, env, node_module_cmd, build_cmd);
        ...
        if (response?.message == "ExpiredAuthenticationToken") {
            ...
            response = await containerBuilderServerJob(git_url, project_slug, NEW_ACCESS_TOKEN, env, node_module_cmd, build_cmd);
